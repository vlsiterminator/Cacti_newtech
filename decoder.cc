...
double Decoder::compute_delays(double inrisetime)
{
  if (exist)
  {
    ...
    //For observing power in each stage. Added temporarily by Ningxi.
    tmp_power[0]=(c_load + c_intrinsic) * Vdd * Vdd;
    ...
    for (i = 1; i < num_gates - 1; ++i)
    {
      ...
      tf = rd * (c_intrinsic + c_load);
    //For observing risetime in each stage. Added temporarily by Ningxi.
      tmp_risetime[i]=inrisetime;
      this_delay = horowitz(inrisetime, tf, 0.5, 0.5, RISE);
      //For observing delay in each stage. Added temporarily by Ningxi.
      tmp_delay[i]=this_delay;
      delay += this_delay;
      inrisetime = this_delay / (1.0 - 0.5);
      power.readOp.dynamic += (c_load + c_intrinsic) * Vdd * Vdd;
      //For observing power in each stage. Added temporarily by Ningxi.
      tmp_power[i]=(c_load + c_intrinsic) * Vdd * Vdd;
    }
    ...
    //For observing power in each stage. Added temporarily by Ningxi.
    tmp_power[i]=c_load * Vpp * Vpp + c_intrinsic * Vdd * Vdd;
